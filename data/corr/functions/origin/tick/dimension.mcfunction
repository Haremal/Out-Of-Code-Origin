...
# STOPPING
execute as @e[tag=!CORR,nbt={Brain:{}}] at @s if dimension corr:out_of_code_dimension run execute unless entity @e[tag=DIM_STOP,distance=..1] run summon minecraft:block_display ~ ~ ~ {Tags:["DIM_STOP","NAN"]}
...
# CORRUPTING
execute as @e[tag=!CORR,nbt={Brain:{}}] at @s if dimension corr:out_of_code_dimension if score NaNBiome Timer matches 0 run scoreboard players add @s Corruption 1
execute as @e[tag=!CORR,nbt={Brain:{}}] at @s if dimension corr:out_of_code_dimension run damage @s 0.0000000001
